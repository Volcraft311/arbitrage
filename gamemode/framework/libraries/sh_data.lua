...
-- @module Arbitrage.data

Arbitrage.data = Arbitrage.data or {}
Arbitrage.data.stored = Arbitrage.data.stored or {}

-- Create a folder to store data in.
file.CreateDir("Arbitrage")

--- Populates a file in the `data/Arbitrage` folder with some serialized data.
-- @realm shared
-- @string key Name of the file to save
-- @param value Some sort of data to save
-- @bool[opt=false] bGlobal Whether or not to write directly to the `data/Arbitrage` folder, or the `data/Arbitrage/schema` folder,
...
-- `data/Arbitrage/schema/map`, where `map` is the name of the current map.
function Arbitrage.data.Set(key, value, bGlobal, bIgnoreMap)
	-- Get the base path to write to.
	local path = "Arbitrage/" .. (bGlobal and "" or "arcade" .. "/") .. (bIgnoreMap and "" or game.GetMap() .. "/")

	-- Create the schema folder if the data is not global.
	if (!bGlobal) then
		file.CreateDir("Arbitrage/" .. "arcade" .. "/")
	end
	...
	Arbitrage.data.stored[key] = value

	return path
end

--- Retrieves the contents of a saved file in the `data/Arbitrage` folder.
-- @realm shared
-- @string key Name of the file to load
-- @param default Value to return if the file could not be loaded successfully
-- @bool[opt=false] bGlobal Whether or not the data is in the `data/Arbitrage` folder, or the `data/Arbitrage/schema` folder,
-- where `schema` is the name of the current schema.
...
function Arbitrage.data.Get(key, default, bGlobal, bIgnoreMap, bRefresh)
	-- If it exists in the cache, return the cached value so it is faster.
	if (!bRefresh) then
		local stored = Arbitrage.data.stored[key]
...
--- Deletes the contents of a saved file in the `data/Arbitrage` folder.
-- @realm shared
...
function Arbitrage.data.Delete(key, bGlobal, bIgnoreMap)
	-- Get the path to read from.
	local path = "Arbitrage/" .. (bGlobal and "" or "arcade" .. "/") .. (bIgnoreMap and "" or game.GetMap() .. "/")
	-- Read the data from a local file.
	local contents = file.Read(path .. key .. ".txt", "DATA")
...
if (SERVER) then
	timer.Create("ArbitrageSaveData", 600, 0, function()
		hook.Run("SaveData")
	end)
end