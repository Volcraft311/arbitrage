...
local function chatTriggerNotSelected()
	chat.AddText(Trigger.prefixColor, Trigger.prefix, color_white, " Триггер не выбран!")
end
...
function TOOL:LeftClick(trace)
	...
	if (!trigger) then 
        chatTriggerNotSelected()
		return false
	end
	trigger:SetPoint(FIRST_POINT, trace.HitPos)
	return true
end
...
function TOOL.BuildCPanel(CPanel)
	...
	TriggerList.OnRowRightClick = function(panel, index, row)
		...
		Menu:AddOption("Первую точку к игроку", function()
			netstream.Start("Trigger:SetPoint", Trigger.selectedID, 1, LocalPlayer():GetPos())
		end):SetIcon("icon16/arrow_in.png")

		Menu:AddOption("Вторую точку к игроку", function()
			netstream.Start("Trigger:SetPoint", Trigger.selectedID, 2, LocalPlayer():GetPos())
		end):SetIcon("icon16/arrow_in.png")
