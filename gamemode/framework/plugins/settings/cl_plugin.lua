...
PLUGIN.type = {
    number = function(data, panel, panelinfo)
        ...
        dslider.Paint = function(_, w, h)
            surface_SetDrawColor(46, 12, 17)
            surface_DrawRect(0, h / 2 - 1, w, 2)
        end
        ...
        sliderPanel.OnValueChanged = function(_, value)
            ...
            if oldValue != value then
                ...
                hook_Run("SETTINGS:OnOptionChange", data.id, value)
            end
    ...
    bool = function(data, panel, panelinfo)
        ...
        buttonNo.Paint = function(_, w, h)
            ...
            surface_SetDrawColor(15, 6, 6, 204 * _.alpha)
        ...
        buttonNo.DoClick = function()
            ...
            hook_Run("SETTINGS:OnOptionChange", data.id, false)
        end
        ...
        buttonYes.DoClick = function()
            ...
            hook_Run("SETTINGS:OnOptionChange", data.id, true)
        end
    end,
    ["string"] = 3,
    bind = function(data, panel, panelinfo)
        ...
        buttonKey.Paint = function(_, w, h)
            local key = input_GetKeyName(data.value) or "NULL"
        ...
        buttonKey.DoClick = function()
            ...
            timer_Create("SETTINGS:ChangeKey", FrameTime(), 0, function()
...
function PLUGIN.ClearTimers()
    timer_Remove("SETTINGS:ChangeKey")
    timer_Remove("SETTINGS:ChangeKeyDelay")
end
...
function PLUGIN.GeneratePanel(data, panel, panelinfo, isBind)
    ...
    frame.DoClick = function()
        ...
        labelDesc.Paint = function(_, w, h)
            for k, v in ipairs(textData) do
                draw_DrawText(v, font, 0, height * k - height, Color(255, 234, 238), TEXT_ALIGN_LEFT)
            end
        end
        ...
        if data.image then
            ...
            imagePanel.Paint = function(_, _w, _h)
                ...
                surface_DrawTexturedRect(0, 0, a2, b2)
    ...
    buttonReset.Paint = function(_, w, h)
        ...
        surface_SetDrawColor(_.color.r, _.color.g, _.color.b)
        surface_SetMaterial(resetMat)
    ...
    buttonReset.DoClick = function()
        ...
        if frame.OnReset then
            if isBind then
                ...
                hook_Run("SETTINGS:OnBindChange", data.id, default)
            else
                local default = SETTINGS.options.GetDefault(data.id)
                frame:OnReset(default)

                hook_Run("SETTINGS:OnOptionChange", data.id, default)
            end
        end
    end
    ...
    line.Paint = function(_, w, h)
        surface_SetDrawColor(255, 234, 238, 10)
